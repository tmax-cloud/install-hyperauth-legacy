...
module.controller('UserListCtrl', function ($scope, realm, User, UserSearchState, UserImpersonation, BruteForce, Notifications, $route, Dialog, $translate, $http) {
    ...
    $scope.removeUser = function (user) {
        Dialog.confirmWithButtonText(
            ...
            function () {
                ...
                $http.delete(authUrl + '/realms/' + realm.realm + '/picture/' + user.username
                ).then(function () {
...
module.controller('UserTabCtrl', function ($scope, $location, Dialog, Notifications, Current, $http) {
    $scope.removeUser = function () {
        Dialog.confirmDelete($scope.user.id, 'user', function () {
            ...
            $http.delete(authUrl + '/realms/' + Current.realm.realm + '/picture/' + $scope.user.username
            ).then(function () {
                console.log("image delete success");
            });
        });
...
module.controller('UserDetailCtrl', function ($scope, realm, user, BruteForceUser, User,
    ...
    $scope.save = function () {
        ...
        if ($scope.create) {
            ...
            if ($scope.deleteFile) {
                $http.delete(authUrl + '/realms/' + realm.realm + '/picture/' + $scope.user.username
                ).then(function () {
                    console.log("image delete success");
