...
<@layout.registrationLayout ; section>
    ...
    <#elseif section = "form">
        <div id="email-sent">
            ...
            <div class="${properties.kcFormGroupClass!}">
                <button class="${properties.kcButtonClass!} ${properties.kcButtonPrimaryClass!} ${properties.kcButtonBlockClass!}" <#if url.loginRestartFlowUrl?has_content>onclick="goToLoginHome('${url.loginRestartFlowUrl}')"<#else>onclick="goToLoginHome(document.location.origin)"</#if>> ${msg("MSG_CREATEACCOUNT_CREATEACCOUNTCOMPLETE_3")}</button>
